...
class single_student:
    ...
    def choose_skill(self, week):
        ...
        if self.review_strat == "random_review":
            ...
            if self.review_strat in ["theta_thres","theta_thres_est"]:
                ...
                acceptable_items = list(self.inv_q_mat[selected_skills[0]])
    ...
    def choose_item(self, selected_block, week):
        ...
        acceptable_items = list(self.inv_q_mat[selected_block])
            ...
            if (self.review_strat != "no_review") & (week > 0):
                for j in range(self.reviews_per_step):
                    ...
                    for b in selected_skill_indices:
                        ...
                        beta_b = self.skill_betas[b] if b < self.B else 0.0
                        win_b = self.list_of_win_params[b] if b < self.B else np.zeros(5)
                        att_b = self.list_of_att_params[b] if b < self.B else np.zeros(5)
                        list_beta.append(beta_b)
                        list_h_features.append((win_b, att_b))
